...
public class ProductController extends BaseController {
	...
	public void list(String addTime,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String recommandFlag,String recipeFlag,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (addTime == null) addTime = "";
		if (productName == null) productName = "";
		if (recommandFlag == null) recommandFlag = "";
		if (recipeFlag == null) recipeFlag = "";
		if(rows != 0)productService.setRows(rows);
		List<Product> productList = productService.queryProduct(addTime, productClassObj, productName, recommandFlag, recipeFlag, page);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(addTime, productClassObj, productName, recommandFlag, recipeFlag);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String addTime,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String recommandFlag,String recipeFlag,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (addTime == null) addTime = "";
		if (productName == null) productName = "";
		if (recommandFlag == null) recommandFlag = "";
		if (recipeFlag == null) recipeFlag = "";
		List<Product> productList = productService.queryProduct(addTime, productClassObj, productName, recommandFlag, recipeFlag, currentPage);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(addTime, productClassObj, productName, recommandFlag, recipeFlag);
	    /*获取到总的页码数目*/
	    int totalPage = productService.getTotalPage();
	    ...
	    request.setAttribute("recipeFlag", recipeFlag);
	    List<ProductClass> productClassList = productClassService.queryAllProductClass();
	    request.setAttribute("productClassList", productClassList);
	...
	public void OutToExcel(String addTime,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String recommandFlag,String recipeFlag, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(addTime == null) addTime = "";
        if(productName == null) productName = "";
        if(recommandFlag == null) recommandFlag = "";
        if(recipeFlag == null) recipeFlag = "";
        List<Product> productList = productService.queryProduct(addTime,productClassObj,productName,recommandFlag,recipeFlag);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Product信息记录"; 
        String[] headers = { "商品id","商品类别","商品名称","商品图片","商品库存","商品价格","是否推荐","是否处方药","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<productList.size();i++) {
        	Product product = productList.get(i); 
        	dataset.add(new String[]{product.getProductId() + "",product.getProductClassObj().getClassName(),product.getProductName(),product.getMainPhoto(),product.getProductNum() + "",product.getPrice() + "",product.getRecommandFlag(),product.getRecipeFlag(),product.getAddTime()});
        }
        /*
